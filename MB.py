...
def uax():
	...
	density = random.choice(['1.0', '1.5', '1.8','2.0', '2.5', '3.0'])
	width = random.choice(["720", "1080", "1280", "1440"])
	height = random.choice(["720", "1080", "1280", "1440", "1520", "1600", "1920", "2400", "2560"])
	fbcr = random.choice(['Teletalk', 'Banglalink', 'Robi', 'Grameenphone', 'Airtel', 'JIO'])
	...
	fbdv = random.choice(["TA-1380", "TA-1380","TA-1356", "TA-1322", "TA-1172","TA-1099", "TA-1099", "TA-1005", "TA-980", "TA-950"])
	fbca = random.choice(["arm64-v8a;", "armeabi-v7a:armeabi"])
	END = f"[FBAN/{str(fban)};FBAV/{str(facebook_version)};FBBV/{str(fbbv)};FBDM/{{density={density},width={width},height={height}}};FBLC/{str(fblc)};FBCR/{str(fbcr)};FBMF/{str(fbmf)};FBBD/{str(fbbd)};FBPN/{str(fbpn)};FBDV/{str(fbdv)};FBSV/{str(fbsv)};FBOP/19;FBCA/{str(fbca)};]"
	ua = f'Davik/2.1.0 (Linux; U; Android {fbsv} '+str(fbdv)+' Build/'+str(build)+') '+END
	return ua
#__________________[ LOGO ]__________________#
logo =f"""{A}    
{A}  EEEEEEE   AAA   BBBBB   RRRRRR    AAA   
{A}  EE       AAAAA  BB   B  RR   RR  AAAAA  
{A}  EEEEE   AA   AA BBBBBB  RRRRRR  AA   AA 
{A}  EE      AAAAAAA BB   BB RR  RR  AAAAAAA 
{A}  EEEEEEE AA   AA BBBBBB  RR   RR AA   AA   {R}❲{G}V{A}/{G}0.1{R}❳{A}
{A}──────────────────────────────────────────────────
{R}❲{G}={R}❳{A} FACEBOOK {R}:{A} MD EABRAHIM MIAH
{R}❲{G}={R}❳{A} TOOLTYPE {R}:{A} FILE {G}&{A} RANDOM CLONING
{R}❲{G}={R}❳{A} STATUS   {R}:{A} PARSONAL
{A}──────────────────────────────────────────────────"""
...
def bd():
		...
		clear()
		print(f'{R}❲{G}1{R}❳{A} METHOD {R}❲{G}M1{R}❳{A} \n{R}❲{G}2{R}❳{A} METHOD {R}❲{G}M2{R}❳  ');linex()
		mthd = input(f'{R}❲{G}?{R}❳{A} CHOICE  {R}:{A} ')
